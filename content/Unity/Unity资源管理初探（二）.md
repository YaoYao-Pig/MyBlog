...
AB包有三层索引，第一层索引是根据Manifest或者自定义的系统找到的AB包，找到AB包之后，AB包是会维护AssetName和Persistent Pointer（持久化指针）(FileIndex,PathID）这个映射关系的。之后用FileIndex找到对应的SeriliaizedFile，第三层是用PathID找到对应的Object。这个FileIndex是全局AB包唯一的
