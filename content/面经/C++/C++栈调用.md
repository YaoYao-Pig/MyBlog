...
### 栈压好之后，IP指针获取数据的时候是可以不按照栈的规则获取，比如可以直接获取某个参数的数值

**栈”这个名字其实容易产生误解。** 它仅仅描述了内存**分配（Allocation）和回收（Deallocation）\**的规则是“先进后出”的，但\**不限制**你如何**使用（Access）**里面的数据

栈就是内存，内存支持随机访问

只有 **`ESP`（栈顶指针）** 的移动通常遵循栈的规则

### 一个栈帧建立之后，EBP自身指向的地址，标记了一个新的栈帧，而EBP指向的地址里存储的数据，是上一个栈帧基址的地址

### 根据这个栈帧的分配和回收逻辑，基本类型POD的局部变量的分配是不初始化，回收也不会清空，类对象特殊的地方是他会调用默认构造函数
